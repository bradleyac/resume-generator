...
var newSourceResumeData = new SourceResumeData(Guid.NewGuid().ToString(), masterResumeData.Resource.UserId, masterResumeData.Resource.Bio, masterResumeData.Resource.Contact, masterResumeData.Resource.Jobs, masterResumeData.Resource.Projects, masterResumeData.Resource.Education, masterResumeData.Resource.Skills, masterResumeData.Resource.Bookshelf);

// await userData.UpsertItemAsync(newSourceResumeData, new PartitionKey(newSourceResumeData.UserId));
...
while (iterator.HasMoreResults)
{
  ...
  bool once = false;
  foreach (var item in response.Where(i => i?.UserId != null)) q
  {
    if (once) return;
...
public record ResumeDataOld(string id, string UserId, bool IsMaster, Bio Bio, Contact Contact, Job[] Jobs, Project[] Projects, Education[] Education, SkillCategory[] Skills, Book[] Bookshelf, Rankings? GeneratedRankings = null, string? CoverLetter = null);
