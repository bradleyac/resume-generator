...
In a nutshell, the Resume Generation System generates 1-page, job-specific resumes and cover letters based on job description text and a source resume. It has an interface for browsing job postings that have been recorded to it and the artifacts associated with that job posting (resume, cover letter, anything else). It has a search mechanism to search the full text within the saved postings. Additionally, when a job posting is saved to the Resume Generation System a process is kicked off which generates a PDF resume and cover letter for that job posting, sending notifications when it is ready.
